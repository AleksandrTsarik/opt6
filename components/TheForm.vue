<template>
  <div class="inspection">
    <div class="inspection__inner">
      <div class="inspection__item">
        <form @submit.prevent="formSubmit">
          <div class="inspection-form">
            <h1 class="inspection-form__title title-b">Проверьте штрафы и зарегестрируйтесь в 1 клик</h1>
            <div class="inspection-form__inputs">
              <UITheInput 
                :type="'text'" 
                :title="'Номер автомобиля'" 
                :name="'number-car'"
                v-model="numberCar"
                :class="onSubmit && numberCar === '' ? 'error-field' : ''"
              />
              <UITheInput 
                :type="'text'" 
                :title="'Регион'" 
                :name="'region-car'"
                v-model="regionCar"
                :class="onSubmit && regionCar === '' ? 'error-field' : ''"
              />
              <UITheInput 
                :type="'text'" 
                :title="'Свидетельство о регистрации ТС'" 
                :name="'ts-car'"
                v-model="tsCar"
                :class="onSubmit && tsCar === '' ? 'error-field' : ''"
              />
            </div>
            <div class="inspection-form__btns">
              <UITheButton 
                :label="'Проверить штрафы'"
                :type="'submit'"
                :svg="svgArrow"
                class="btn-dark"
              />
              <UITheButton 
                :label="'О сервисе'"
                :svg="svgPlay"
                :time="time"
                class="btn-light"
                @click.prevent="isModalVideo = true"
              />  
            </div>
            <div class="inspection-form__agreement">
              Нажимая «Проверить штрафы» вы соглашаетесь&nbsp;с политикой обработки персональных данных и принимаете оферту
            </div>
          </div>
        </form>
      </div>
      <div class="inspection__item">
        <div class="inspection__img">
          <img src="@/assets/img/comp.png" alt="">
        </div>
      </div>
    </div>
  </div>
  <the-modal-video v-if="isModalVideo" @close="closeModal"></the-modal-video>
</template>

<script>
import TheModalVideo from './modal/TheModalVideo.vue'
export default {
  components: { TheModalVideo },
  data() {
    return {
      tsCar: '',
      numberCar: '',
      regionCar: '',
      onSubmit: false,
      isModalVideo: false,
      svgArrow: '<svg width="15" height="13" viewBox="0 0 15 13" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M11.9226 6.67278L0.570412 6.66773C0.257929 6.66759 0.00472069 6.41416 0.00485992 6.10168L0.00487518 6.09709C0.00757599 5.781 0.26404 5.52588 0.580147 5.52485L11.9083 5.48784L7.46464 1.01635C7.23486 0.785138 7.23115 0.412947 7.45628 0.177198C7.67473 -0.051569 8.03728 -0.0599298 8.26604 0.158525C8.26915 0.161493 8.27222 0.164496 8.27526 0.167534L13.8712 5.75744C14.0666 5.9526 14.0668 6.26918 13.8716 6.46455L13.87 6.4662L8.18914 12.1003C7.95859 12.329 7.58633 12.3275 7.35767 12.0969C7.35452 12.0937 7.3514 12.0905 7.34832 12.0873C7.11811 11.8443 7.12347 11.4621 7.36039 11.2257L11.9226 6.67278Z" fill="white"/></svg>',
      svgPlay: '<svg width="32" height="23" viewBox="0 0 32 23" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2.5258 20.2492C1.61798 19.0743 1.16406 16.1905 1.16406 11.5976C1.16406 7.00468 1.52855 4.25596 2.25754 3.35141C2.75216 2.6247 3.22423 2.1221 3.67376 1.8436C4.12329 1.5651 4.71528 1.40847 5.44973 1.37371C10.5243 1.12406 14.3824 0.999492 17.0239 1C19.6655 1.00051 23.1675 1.12651 27.5299 1.37798C28.2271 1.51283 28.8478 1.77363 29.392 2.16038C29.9363 2.54714 30.3022 3.04098 30.4897 3.64191C31.0955 5.04118 31.3984 7.52884 31.3984 11.1049C31.3984 14.6809 31.1736 17.2829 30.7239 18.9108C30.4927 19.803 30.0488 20.5145 29.392 21.045C28.7353 21.5756 27.852 21.8344 26.7423 21.8215C21.892 22.0624 18.2543 22.1787 15.8292 22.1702C13.4042 22.1618 9.93055 22.0209 5.40843 21.7474C4.7907 21.7474 4.26489 21.6434 3.83098 21.4353C3.39708 21.2272 2.96202 20.8318 2.5258 20.2492Z" stroke="#1253A2"/><path fill-rule="evenodd" clip-rule="evenodd" d="M13.2152 7.03704L13.219 7.04004L21.3701 11.3065L20.3739 11.7778L13.2089 15.4787L13.2149 7.03804L13.2152 7.03704Z" fill="#1253A2"/></svg>',
      time: '(10:23)'
    }
  },
  methods: {
    closeModal() {
      this.isModalVideo = false
    },
    formSubmit() {
      this.onSubmit = true
    }
  },
}
</script>

<style lang="scss">
  .inspection {
    margin-top: 80px;
    &__inner {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 40px;
      @media(max-width: 1023px) {
        grid-template-columns: 1fr;
      }
    }
    &__item {
      &:first-child {
        @media(max-width: 1023px) {
          order: 2;
        }
      }
      &:last-child {
        @media(max-width: 1023px) {
          order: 1;
          margin: 0 auto;
        }
      }
    }
  }
.inspection-form {
  &__title {
    margin-bottom: 25px;
  }
  &__inputs {
    display: grid;
    grid-template-areas:
        "a a b"
        "c c c";
    gap: 15px 30px;
    @media(max-width: 575px) {
      gap: 5px 10px;
    }
    .block-input {
      &:nth-child(1) {
        grid-area: a;
      }
      &:nth-child(2) {
        grid-area: b;
      }
      &:nth-child(3) {
        grid-area: c;
      }
    }
  }
  button {
    display: flex;
    justify-content: center;
    @media(max-width: 575px) {
      width: 100%;
    }
  }
  &__btns {
    display: flex;
    margin-top: 15px;
    margin-bottom: 15px;
    @media(max-width: 575px) {
      flex-wrap: wrap;
    }
    span {
      display: flex;
      position: relative;
      justify-content: center;
      align-content: center;
      svg {
      }
    }
    .btn-dark {
      display: flex;
      flex-direction: row-reverse;
      align-items: center;
      margin-right: 15px;
      @media(max-width: 575px) {
        margin-right: 0;
        margin-bottom: 20px;
      }
      span {
        margin-left: 10px;
        margin-bottom: -4px;
      }
    }
    .btn-light {
      display: flex;
      align-items: center;
      font-size: 18px;
      span {
        &:first-child {
          padding-right: 5px;
          //margin-bottom: -3px;
        }
        &:last-child {
          color: #1253A2;
          padding-left: 5px;
        }
      }
    
    }
    button {
      @media(max-width: 575px) {
        display: block;
        width: 100%;
        text-align: center;
        height: 50px;
        line-height: 50px;
        padding: 0;
      }
    }
  }
  &__agreement {
    color: #8F8F8F;
    font-size: 13px;
  }
}
</style>